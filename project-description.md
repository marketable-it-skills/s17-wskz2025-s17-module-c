...
You need to develop a REST API for a classifieds service — a geek marketplace. This is a platform where users can post and browse ads for selling LEGO sets, board games, books, souvenirs, and similar items.

In this module, you must implement role-based access for three roles: advert author, guest, and moderator.

## General Description of Project and Tasks

- A database dump is not provided; you must create the structure and populate the tables yourself.
- All endpoints and validation rules are described in the OpenAPI specification file [`assets/swagger/openapi.yaml`](assets/swagger/openapi.yaml).
- A Postman collection for API testing is provided at [`assets/collection.json`](assets/collection.json).
- Create the following accounts and include them in the database dump:
...
- In the `database` directory of your project, store the database dump (`DB_DUMP.sql`) and the ER diagram (`ERD.png`).
- In the database dump, add test data for all tables so that the auto-tests pass successfully.
- Create a `README.md` file in your project so evaluators can understand your solution and any new endpoints you add.

## Requirements

Develop a REST API that fulfils the specifications below and in the provided OpenAPI file.
...
You must also design endpoints for enabling, extending, and retrieving services, and update the **openapi.yaml** file accordingly. Save it in the `swagger` directory of your project.

## Assessment

- API behaviour is verified against the OpenAPI specification ([`assets/swagger/openapi.yaml`](assets/swagger/openapi.yaml)).
- Endpoints are tested manually and with the provided Postman collection ([`assets/collection.json`](assets/collection.json)).
- Automated tests check database structure, seed data, authentication, advert workflows, and promotion services.
- Database deliverables (`DB_DUMP.sql`, `ERD.png`) are reviewed for completeness and correct test data.
- New or extended endpoints for advantage services are checked against the competitor's updated OpenAPI file and project `README.md`.
